...
if(_iCountItems > 0) then {
	...
	{		
		{pdb_template_interface addAction[format["Load: %1",(_sql_res select _i+1)],{ {[(_sql_res select _i)] execVM "\pdb\functions\templates\pdb_load_template.sqf"} remoteExec ["call",2]; }];} remoteExec ["call",-2];
	};
	...
	//[_object, ["<t color='#00FF33'>SAVE</t>", {[[[]],"\pdb\functions\templates\pdb_save_template.sqf"],"BIS_fnc_execVM"] remoteExec ["call",2,false]}] remoteExec ["addAction", -2, _object];
		...
		{pdb_template_interface addAction[format["--REFRESH--",{ {[] execVM "\pdb\functions\templates\pdb_load_template_actions.sqf"} remoteExec ["call",2]; }];} remoteExec ["call",-2];