...
if(_iCountItems > 0) then {
	...
	{		
		...
		[format["Load: %1",(_sql_res select _i+1)],{[[(_sql_res select _i)],"\pdb\functions\templates\pdb_load_template.sqf","BIS_fnc_execVM"] remoteExec["call", 2]}] remoteExec["addAction", -2, _object];
	};
