	...
	if(isNil "_disableFlag") then {
		...
		if(_objClass in sru_allowed_Obj && !(_objType in sru_blacklist_Obj)) then {
			...
			if(isNil "_objUID") then {
				"extDB3" callExtension format["0:SQL:INSERT INTO objects SET objName = '%1', objClass = '%2', objPos = '%3', objDir = '%4',objDamage = '%5',mission_FK = '%6',objRecDate = NOW(), objCategory = '%7', objInit = '%8'", _objName,_objType,_objPos,_objDir,_objDamage,sru_pdb_mission_fk,_objClass,_objInit];
