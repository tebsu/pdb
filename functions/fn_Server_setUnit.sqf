...
"extDB3" callExtension format["0:SQL:INSERT IGNORE INTO player_mission_connection SET playerUID = '%1',mission_FK = '%2'",_playerUID,sru_pdb_mission_fk];

if(count(_playerInv) > 70) then {
	"extDB3" callExtension format["0:SQL:UPDATE player_mission_connection SET playerInv = '%1', playerPos = '%2', playerDir = '%3',playerLastOnline = NOW() WHERE playerUID = '%5' AND mission_FK = '%4'",_playerInv,_playerPos,_playerDir,sru_pdb_mission_fk,_playerUID];
}else{
	"extDB3" callExtension format["0:SQL:UPDATE player_mission_connection SET playerPos = '%1', playerDir = '%2',playerLastOnline = NOW() WHERE playerUID = '%4' AND mission_FK = '%3'",_playerPos,_playerDir,sru_pdb_mission_fk,_playerUID];
};